        ...
        case "POST":
            try {
                ...
                uploadMiddleware(req, res, async (err) => {
                    ...
                    const result = await db.collection("ameneties").insertOne({
                        name,
                        short_name,
                    ...
                    return res.status(201).json({
                        message: "Amenety added successfully",
                        result,
                    });
                });
                break;
        ...
        case "PUT":
            try {
                ...
                uploadMiddleware(req, res, async (err) => {
                    ...
                    // Save the data to MongoDB
                    const result = await db.collection("ameneties").updateOne(
                        { _id: new ObjectId(id) },
                        {
                            $set: {
                                name,
        ...
        case "DELETE":
            try {
                ...
                if (result.deletedCount > 0) {
                    return res
                        .status(200)
                        .json({ message: "Amenity deleted successfully" });
                } else {
                    return res
                        .status(404)
                        .json({ message: "Amenity not found" });
                }
            } catch (error) {
